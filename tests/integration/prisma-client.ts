 ...
 * through the same `prismaAdapterFromUrl` the app uses is what keeps "a second
 * connection like the app's" true rather than approximately true — including
 * a `?schema=`, which the adapter only honours when handed it explicitly.
 */
export function newTestClient(url: string = process.env.DATABASE_URL ?? ''): PrismaClient {
  return new PrismaClient({ adapter: prismaAdapterFromUrl(url) });
}
